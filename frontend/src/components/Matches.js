    ...
    const startConversation = async (matchId) => {
        try {
            ...
            if (data.status === 'success') {
                window.location.href = `/app/chat/${data.conversation._id}`;
            }
        } catch (error) {
                        ...
                        {matches.map((match, index) => (
                            ...
                            >
                                <div className="relative">
                                    {match.otherUser?.profilePicture ? (
                                        <img
                                            src={match.otherUser.profilePicture.startsWith('http') 
                                                ? match.otherUser.profilePicture 
                                                : `${process.env.REACT_APP_API_URL}${match.otherUser.profilePicture}`
                                            }
                                    ...
                                    <div className="w-full h-48 bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center" style={{display: match.otherUser?.profilePicture ? 'none' : 'flex'}}>
                                ...
                                <div className="p-6">
                                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                                        {match.otherUser?.name}, {match.otherUser?.age || 'N/A'}
                                    </h3>
